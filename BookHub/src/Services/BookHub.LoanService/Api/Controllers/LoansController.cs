...
public class LoansController : ControllerBase
{
    ...
    public async Task<ActionResult<LoanDto>> Create([FromBody] CreateLoanDto dto, CancellationToken cancellationToken)
    {
        try
        {
            var createdLoan = await _loanService.CreateLoanAsync(dto, cancellationToken);
    ...
    {
        var returnedLoan = await _loanService.ReturnLoanAsync(id, cancellationToken);
        if (returnedLoan == null) return NotFound();
